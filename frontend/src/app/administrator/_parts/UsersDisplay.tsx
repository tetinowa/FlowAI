...
export function UsersDisplay() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-muted/30 text-foreground">
      <div>
        <h2 className="text-3xl font-black text-foreground tracking-tight">Users</h2>
      ...
      <div className="flex flex-col gap-3">
        {MOCK_USERS.map((user) => (
          ...
          >
            ...
            <Badge variant="outline" className={`${roleColors[user.role] ?? ""} text-xs shrink-0`}>
              {user.role}
            </Badge>

            <p className="text-muted-foreground text-xs shrink-0 w-24 text-right">
              Joined {user.joinedSince}
            </p>

            {deleteConfirmId === user.id ? (
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-destructive text-xs">Delete?</span>
                <Button size="sm" variant="destructive" className="h-7 text-xs px-2" onClick={() => handleDelete(user.id)}>
                  Yes
                </Button>
                <Button size="sm" variant="ghost" className="h-7 text-xs px-2" onClick={() => setDeleteConfirmId(null)}>
                  No
                </Button>
              </div>
            ) : (
        ...
        <SheetContent className="w-105 p-6">
          ...
          <div className="mt-6 flex flex-col gap-4">
            ...
            <div className="flex gap-2 pt-2">
              <Button onClick={handleEditSubmit} className="flex-1 bg-[#5048e5] hover:bg-[#4038d4] text-white">
                Save Changes
              </Button>
